...
function Player(player) {
    const img = Widget.Box({
        class_name: "img",
        visible: player.bind("cover_path").as(p => !!p),
        vpack: "start",
    ...
    const title = Widget.Label({
        ...
        wrap: false,
        hpack: "start",
        label: player.bind("track_title").as(t => {
            const track_artists = player.track_artists.join(', ')
            if (track_artists === 'Unknown artist') return t
            !!t && Utils.notify({
                summary: track_artists,
    ...
    return Widget.Box(
        { class_name: "player", visible: player.bind("can_play") },
        img,
        Widget.Box(
            {
                vertical: true,
...
const mediaWidget = Widget.EventBox({
    on_primary_click: () => {
